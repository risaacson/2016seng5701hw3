...
>
  ...
  <!-- xref -->
  <!-- set to no if you don't want page numbers -->
  <xsl:param name="insert.xref.page.number">yes</xsl:param>

  <!-- Generated Text changes -->
   <xsl:param name="collect.xref.targets">no</xsl:param>

   <!-- When I make workbooks I like to use "exercises" - here's an
        example of how you can change the text associated with a 
        tag, so that whenever we use the <procedure> markup we get
        "Exercise" output.
  ...
  <!-- line break support -->
  <!--  add <?linebreak?> to your markup if you need to force breaks.  -->
  ...
  <xsl:template match='d:command'>
    <!-- uncomment if you want to force a $ in front of the output -->
    <!-- <xsl:text>$ </xsl:text> -->
  ...
  <!-- methodname, like a method call on an object. -->
